...
from DFA3D_V2.utils.io import _load, _numpy_to_cuda, _numpy_to_tensor
...
def convert_bfm_to_onnx(bfm_onnx_fp, shape_dim=40, exp_dim=10):
    ...
    bfm_decoder = BFMModel_ONNX(
        bfm_fp=bfm_fp, shape_dim=shape_dim, exp_dim=exp_dim
    )
    bfm_decoder.eval()
    ...
    dummy_input = (
        torch.randn(3, 3),
        torch.randn(3, 1),
        torch.randn(shape_dim, 1),
        torch.randn(exp_dim, 1),
    )
