...
from DFA3D_V2.bfm.bfm import BFMModel
from DFA3D_V2.bfm.bfm_onnx import convert_bfm_to_onnx
from DFA3D_V2.utils.functions import (
    crop_img,
    parse_roi_box_from_bbox,
    parse_roi_box_from_landmark,
)
from DFA3D_V2.utils.io import _load
from DFA3D_V2.utils.onnx import convert_to_onnx
from DFA3D_V2.utils.tddfa_util import _parse_param, similar_transform
...
class TDDFA_ONNX(object):
    ...
    def __init__(self, **kvs):
        ...
        onnx_fp = kvs.get(
            "onnx_fp", kvs.get("checkpoint_fp").replace(".pth", ".onnx")
        )

        # convert to onnx online if not existed
        if onnx_fp is None or not osp.exists(onnx_fp):
            print(
                f"{onnx_fp} does not exist, try to convert the `.pth` version"
                " to `.onnx` online"
            )
